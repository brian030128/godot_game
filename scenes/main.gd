extends Node2D
## Root of the playable prototype. Wires the joystick and skill buttons to the
## player (dependency injection), assigns the player's skills, points the camera
## at the player, and spawns the initial wave of mobs. The player auto-attacks,
## so there is no attack button to wire.
...
## Skills assigned to the player's slots, in button order (slot 0 = dash, shown
## as the large dedicated dash button). A null entry leaves that button
## empty/inert. The 4th slot is reserved for later.
